...
import {
  INPUT_UPGRADES_CONDITIONS,
  UnlockConditionFunc,
} from "../upgrades/unlock-conditions";
  ...
  create() {
      ...
      (currentPlayerProgress: PlayerProgress) => {
        ALL_INPUT_UPGRADES.filter(
          ...
          if (unlockCondition(currentPlayerProgress.wordsFound)) {
            if (upgrade !== InputUpgrades.CAT) {
              eventsCenter.emit("UNLOCK_UPGRADE", upgrade);
            } else {
              eventsCenter.emit("UNLOCK_ALL_UPGRADES");
            }
    ...
    this.events.on(
      ...
      (upgrade: InputUpgrades) => {
        this.bannerContainer = new UnlockBanner(this.scope, 0, 0, upgrade);
